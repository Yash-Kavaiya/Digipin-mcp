 ...
 */
export function decodeDIGIPIN(digipin: string): { lat: number; lon: number } {
  // Remove hyphens
 ...
 */
export function validateDIGIPIN(digipin: string): { valid: boolean; error?: string } {
  const cleanDigipin = digipin.replace(/-/g, '');
 ...
 */
export function getDIGIPINInfo(digipin: string): {
  code: string;
